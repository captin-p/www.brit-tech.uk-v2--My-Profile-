...
export default async function GuidePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params; const guide = findGuide(slug); if (!guide) notFound();
  ...
  const related = allGuides.filter((candidate) => candidate.slug !== guide.slug).map((candidate) => ({ guide: candidate, score: candidate.tags.filter((tag) => guide.tags.includes(tag)).length + (candidate.category === guide.category ? 1 : 0) })).filter(({ score }) => score > 0).sort((a, b) => b.score - a.score).slice(0, 3).map(({ guide: item }) => item);
  ...
  return <main className="mx-auto max-w-4xl px-6 py-16 md:py-24">
    ...
    <article className="mt-12 space-y-10">{guide.sections.map((section) => <section key={section.heading}><h2 className="text-2xl font-semibold">{section.heading}</h2>{section.body.map((paragraph, index) => <p key={index} className="mt-4 text-base leading-8 text-muted-foreground">{paragraph}</p>)}</section>)}</article>
    ...
    {related.length > 0 && <section className="mt-10"><h2 className="text-2xl font-semibold">Related BritTech Guides</h2><div className="mt-4 grid gap-4 md:grid-cols-3">{related.map((item) => <Link key={item.slug} href={`/guides/${item.slug}`} className="rounded-2xl border p-5 transition hover:bg-muted"><p className="text-sm font-semibold leading-6">{item.title}</p><p className="mt-2 text-xs text-muted-foreground">{item.minutes} min · {item.difficulty}</p></Link>)}</div></section>}
    <aside className="mt-10 rounded-2xl border p-6"><h2 className="font-semibold">Need professional help?</h2><p className="mt-2 text-sm text-muted-foreground">If the guide does not resolve the problem, stop before making risky hardware, electrical or destructive configuration changes.</p><Link href="/contact" className="mt-4 inline-block text-sm font-semibold underline">Contact Bright</Link></aside>
  </main>;
}
